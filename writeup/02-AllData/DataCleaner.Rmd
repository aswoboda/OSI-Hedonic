...
require(foreign)
getwd()
HouseData = read.dbf("../../../Data/GIS2R/allData.dbf") # this file address requires that R has the default working directory of the github repository "OSI-Hedonic"
```
...
```{r, cache=TRUE}
neighborhoodModel1 = lm(logPRICE ~ FIN_SQ_FT + ACRES_POLY  + YEAR_BUILT + HOME_STYLE +
...
```{r, cache=TRUE}
neighborhoodModel2 = lm(logPRICE ~ FIN_SQ_FT + ACRES_POLY + YEAR_BUILT + HOME_STYLE +
...
```{r, cache=TRUE}
TimeModel1 = lm(logPRICE ~ FIN_SQ_FT + ACRES_POLY  + YEAR_BUILT + HOME_STYLE + ELEMENTARY + WhiteDense + fam_income + factor(SALE_YEAR) + factor(SALE_MONTH), 
            data = HouseData)
summary(TimeModel1)
```
...
```{r, cache=FALSE}
osiModel05KM.2 = lm(logPRICE ~ FIN_SQ_FT + ACRES_POLY  + YEAR_BUILT + HOME_STYLE + ELEMENTARY + WhiteDense + factor(SALE_YEAR) + factor(SALE_MONTH) +
parkArea05, 
            data = HouseData)
